...
pub async fn run(cli: &Cli) -> anyhow::Result<()> {
    ...
    if cli.concurrent <= 0 {
        write_error!(
            "Concurrent requests cannot be lower than 1: {} is given\n",
            cli.concurrent
        );
    ...
    let client = HttpClient::new(
        ...
        Duration::from_secs(cli.timeout.max(1) as u64),
...
fn print_version() {
    ...
    write_out!("https://github.com/librespeed/speedtest-cli\n");
...
fn apply_scheme(raw: &str, force: ForceScheme) -> String {
    ...
    stream::iter(servers.iter().enumerate())
        .map(|(idx, server)| async move {
            ...
            if !server.is_up(ctx.client, &tlog).await {
                write_debug!(
                    "Server {} ({hostname}) doesn't seem to be up, skipping\n",
                    server.name
                );
                return None;
            }
                ...
                .await
            {
                ...
                Err(_) => {
                    write_debug!("Can't ping server {} ({hostname}), skipping\n", server.name);
                    None
                }
            }
        })
