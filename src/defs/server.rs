...
impl Server {
    ...
    pub fn get_url(&self) -> anyhow::Result<Url> {
        Url::parse(&self.server).with_context(|| format!("invalid server URL: {}", self.server))
    }
    ...
    pub fn sponsor(&self) -> String {
        ...
        if !self.sponsor_url.is_empty() {
            ...
            match Url::parse(&url) {
                ...
                Err(_) => write_debug!("Sponsor URL is invalid: {}\n", self.sponsor_url),
            }
        }
        msg
    }
    ...
    pub async fn icmp_ping_and_jitter(
        ...
        if rtts.is_empty() {
            write_debug!(
                "No ICMP pings returned for server {} ({host}), trying TCP ping\n",
                self.name
            );
            return self.ping_and_jitter(client, tlog, count + 2).await;
        }
