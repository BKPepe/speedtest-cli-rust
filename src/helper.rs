...
pub async fn do_speed_test(
    ...
    for current_server in servers {
        ...
        write_ui!("Selected server: {} [{}]\n", current_server.name, hostname);

        let sponsor_msg = current_server.sponsor();
        if !sponsor_msg.is_empty() {
            write_ui!("Sponsored by: {sponsor_msg}\n");
        }

        if !current_server.is_up(ctx.client, &tlog).await {
            write_ui!(
                "Selected server {} ({hostname}) is not responding at the moment, try again later\n",
                current_server.name
            );
        ...
        write_ui!("You're testing from: {}\n", isp_info.processed_string);
...
async fn send_telemetry(
    ...
    if parts.len() != 2 {
        anyhow::bail!("server returned invalid response: {response}");
    }
